class SubCategoriesController < ApplicationController
  ...
  def index
    ...
    if params[:limit]
      @sub_categories = @q.result.page(params[:page]).per(params[:limit])

   else
     @sub_categories = @q.result.page(params[:page]).per(10)
   end
  end
  ...
  private
    ...
    def sub_category_params
      params.require(:sub_category).permit(:cat_id, :sub_cat_descrip)
    end
end
